...
class TwoStageTrainer:
    """Two-stage trainer: DFM + DirectPoseMLP"""
    
    def __init__(self, cfg, dfm_pretrained_path=None, mlp_pretrained_path=None, translation_status=None):
        self.cfg = cfg
        ...
        # Stage 2: DirectPoseMLP (direct pose prediction)
        self.mlp = DirectPoseMLP(
            cfg, device=self.device
        ).to(self.device)
        if mlp_pretrained_path is not None:
            print(f"Loading DirectPoseMLP from {mlp_pretrained_path}")
            self.mlp.load_state_dict(torch.load(mlp_pretrained_path, map_location=self.device))
            print("DirectPoseMLP loaded successfully")
        ...
        # Optimizer (only optimize MLP if DFM is frozen)
        if self.freeze_dfm:
            ...
            self.optimizer = torch.optim.RAdam(
                list(self.dfm.parameters()) + list(self.mlp.parameters()),
                lr=cfg.lr,
                betas=(0.95, 0.999),
                weight_decay=1e-6
            )
        ...
        self.ema = ExponentialMovingAverage(
            self.mlp.parameters() if self.freeze_dfm else list(self.dfm.parameters()) + list(self.mlp.parameters()),
    ...
    def train_step(self, batch_sample):
        ...
        # Transform point cloud by coarse pose (inverse transform) for MLP
        ...
        # Extract point cloud features for MLP
        acfm_pts_feat = self.mlp.extract_pts_feature(pts_transformed).to(self.device)
        ...
        acfm_loss, acfm_loss_desc, loss_dict = self.mlp.loss(pose_gt, acfm_pts_feat, topk_info, self.translation_status)
        
        # Backward and optimize
        self.optimizer.zero_grad()
        acfm_loss.backward()
        grad_norm = torch.nn.utils.clip_grad_norm_(self.mlp.parameters(), 1.0)
        self.optimizer.step()
        self.ema.update(self.mlp.parameters() if self.freeze_dfm else list(self.dfm.parameters()) + list(self.mlp.parameters()))
    ...
    def eval_step(self, batch_sample):
        ...
        self.ema.store(self.mlp.parameters() if self.freeze_dfm else list(self.dfm.parameters()) + list(self.mlp.parameters()))
        self.ema.copy_to(self.mlp.parameters() if self.freeze_dfm else list(self.dfm.parameters()) + list(self.mlp.parameters()))
        ...
        try:
            with torch.no_grad():
                ...
                pred_pose = self.mlp.sample(acfm_pts_feat, topk_info, self.translation_status, step_size=self.cfg.T_acfm, method='euler')
            ...
            self.ema.restore(self.mlp.parameters() if self.freeze_dfm else list(self.dfm.parameters()) + list(self.mlp.parameters()))
...
def train_two_stage(cfg, train_loader, val_loader, test_loader, translation_status=None):
    ...
    mlp_path = cfg.mlp_pretrained_path if hasattr(cfg, 'mlp_pretrained_path') else None
    
    trainer = TwoStageTrainer(
        cfg, 
        dfm_pretrained_path=dfm_path, 
        mlp_pretrained_path=mlp_path,
    ...
    wandb.watch(trainer.mlp, log="all", log_freq=100)
            ...
            for batch in pbar:
                ...
                if current_step % eval_freq == 0:
                    ...
                    save_path = os.path.join(
                        save_dir,
                        f"mlp_step_{current_step}_angle_{stage2_angle_mean:.4f}_trans_{stage2_trans_mean:.4f}.pt"
                    )
                    # Store current params and copy EMA params for saving
                    trainer.ema.store(trainer.mlp.parameters() if trainer.freeze_dfm else list(trainer.dfm.parameters()) + list(trainer.mlp.parameters()))
                    trainer.ema.copy_to(trainer.mlp.parameters() if trainer.freeze_dfm else list(trainer.dfm.parameters()) + list(trainer.mlp.parameters()))
                    torch.save(trainer.mlp.state_dict(), save_path)
                    trainer.ema.restore(trainer.mlp.parameters() if trainer.freeze_dfm else list(trainer.dfm.parameters()) + list(trainer.mlp.parameters()))
