...
class DiscreteFlowTest:
    ...
    def test_step(self, batch_sample):
        ...
        with torch.no_grad():
            pred = self.model.sample(pts_feat, step_size=0.01)
...
class DiscreteFlowTrainer:
    def __init__(self, cfg, trans_stats=None,pretrained_path=None):
        ...
        #             self.optimizer,
        #             factor=1.0,
        #             total_iters=int(cfg.n_epochs * 0.5)  
        #         ),

        #         torch.optim.lr_scheduler.CosineAnnealingLR(
        #             self.optimizer,
        #             T_max=int(cfg.n_epochs * 0.5),
        #             eta_min=5e-7  # 稍微调高最小学习率
        #         )
        #     ],
        #     milestones=[int(cfg.n_epochs * 0.6)]  # 延迟学习率下降
        # )
...
def train_data(cfg, train_loader, val_loader, test_loader,trans_stats):
    ...
    trainer = DiscreteFlowTrainer(cfg, trans_stats,pretrained_path=None)
    wandb.watch(trainer.model, log="all", log_freq=100)
    save_dir = "DiscreteFlow_6D"
...
def Test_data(cfg, test_loader, trans_stats):
    ...
    Test_Model = DiscreteFlowTest(cfg, trans_stats,pretrained_path=pretrained_path)
...
def main():
    ...
    wandb.init(project="discrete_flow_matching", config={
        "batch_size": 96,
        "n_epochs": 200,
        "learning_rate": 3e-4,
        "encoder": "pointnet2",
